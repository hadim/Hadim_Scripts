...
@Plugin(type = ContextCommand.class,
	menuPath = "Plugins>Hadim>Pseudo Flat-Field Correction")
public class PseudoFlatFieldCorrectionCommand extends
	AbstractPreprocessingCommand
{
	...
	@Parameter(type = ItemIO.INPUT, min = "0",
		label = "Gaussian Filter Size (pixel)",
	...
	@Parameter(type = ItemIO.INPUT,
		label = "Iterate over XY planes (reduce memory usage)", required = false)
	...
	public void run() {
		...
		if (this.iteratePlane) {
			this.output = doPseudoFlatFieldCorrectionXYPlaneIterate(this.input,
				this.gaussianFilterSize);
		}
		else {
			this.output = doPseudoFlatFieldCorrection(this.input,
				this.gaussianFilterSize);
		}
	...
	public <T extends RealType<T>> Dataset
		doPseudoFlatFieldCorrectionXYPlaneIterate(Dataset input,
			double gaussianFilterSize)
	{
		...
		if (dataset.dimension(Axes.Z) > 1) {
			log.error(
				"The \"iterate over XY planes\" option cannot be used with more " +
		...
		for (int i = 0; i < intervals.size(); i++) {
			...
			// // Convert to 32 bit
			floatPlane = (IterableInterval<FloatType>) ops.run("convert.float32",
				plane);

			// Estimate the background
			background = (IterableInterval<FloatType>) ops.run("filter.gauss",
				floatPlane, sigmas);
			...
			// Do the subtraction
			ops.math().subtract(subtracted, (IterableInterval) floatPlane,
				(IterableInterval) background);

			// Convert back to the original type
			converter = (RealTypeConverter) ops.op("convert.clip", dataset
				.getImgPlus().firstElement(), subtracted.firstElement());
		...
		if (dataset.numDimensions() != im.numDimensions()) {
			...
			Interval interval1 = Intervals.createMinMax(0, 0, 0, im.dimension(0) - 1,
				im.dimension(1) - 1, middleDimension - 1);
			Interval interval2 = Intervals.createMinMax(0, 0, middleDimension, im
				.dimension(0) - 1, im.dimension(1) - 1, im.dimension(2) - 1);

			RandomAccessibleInterval offsetInterval1 = Views.offsetInterval(im,
				interval1);
			RandomAccessibleInterval offsetInterval2 = Views.offsetInterval(im,
				interval2);
	...
	public <T extends RealType<T>> Dataset doPseudoFlatFieldCorrection(
		Dataset input, double gaussianFilterSize)
	{
		Dataset dataset = input.duplicate();
		...
		// Convert to 32 bits
		IterableInterval<FloatType> out = (IterableInterval<FloatType>) ops.run(
			"convert.float32", dataset.getImgPlus());
		IterableInterval<FloatType> original = (IterableInterval<FloatType>) ops
			.run("convert.float32", dataset.getImgPlus());
		IterableInterval<FloatType> backgroundFloat =
			(IterableInterval<FloatType>) ops.run("convert.float32", background
				.getImgPlus());

		// Do subtraction
		status.showStatus("Subtract image to background.");
		IterableInterval<FloatType> out2 = (IterableInterval<FloatType>) ops
			.create().img(out);
		...
		if (normalizeIntensity) {
			...
			RealTypeConverter scaleOp = (RealTypeConverter) ops.op(
				"convert.normalizeScale", out4.firstElement(), out3.firstElement());
